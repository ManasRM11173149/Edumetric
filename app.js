...
/* ----- CBSE Syllabus helpers ----- */
function fillSubjectSelect(el, grade) {
    if (!el) return;
    if (!grade || !CBSE_SYLLABUS[grade]) {
        el.innerHTML = `<option value="">-- Select a grade first --</option>`;
        return;
    }
    const subjects = Object.keys(CBSE_SYLLABUS[grade].subjects);
    let html = `<option value="">-- Select Subject --</option>`;
    subjects.forEach(s => { html += `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`; });
    el.innerHTML = html;
}

function fillCBSETopicSelect(el, grade, subject) {
    if (!el) return;
    if (!grade || !subject || !CBSE_SYLLABUS[grade] || !CBSE_SYLLABUS[grade].subjects[subject]) {
        el.innerHTML = `<option value="">-- Select a subject first --</option>`;
        return;
    }
    const topics = CBSE_SYLLABUS[grade].subjects[subject];
    let html = `<option value="">-- Select Topic --</option>`;
    topics.forEach(t => { html += `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`; });
    el.innerHTML = html;
}
...
function getQuestionsFor(grade, topic, count) {
    ...
    // Create a shuffled list of indices to sample without immediate repetition
    let availableIndices = shuffle(Array.from({ length: basePool.length }, (_, i) => i));

    // Draw questions until we have enough
    while (result.length < wanted) {
        // If we've used all indices, reshuffle and continue (but avoid immediate duplicates)
        if (availableIndices.length === 0) {
            availableIndices = shuffle(Array.from({ length: basePool.length }, (_, i) => i));
        }

        const idx = availableIndices.shift();
        result.push(reshuffleOptions(basePool[idx]));
        usedIndices.add(idx);
    }
    return result;
}
...
/* ============ Cross-subject question generation (Create Quiz) ============
   ELA_QUIZ_BANK only holds curated content for English topics. The
   "Create Quiz" flow lets a teacher pick ANY CBSE subject + topic, so for
   everything the curated bank doesn't cover, we synthesize grade-appropriate
   starter questions the teacher can review/edit before saving — the same
   "generate then edit" pattern the ELA flow already uses. Math topics get
   real, randomly-generated arithmetic problems (verifiably correct, and
   effectively unlimited so any question count works); every other subject
   gets structured prompts built from the subject/topic/grade so the quiz is
   never empty, no matter what's picked or how many questions are asked. */

function randInt(min, max) {
    return Math.floor(Math.random() * (max - min + 1)) + min;
}

/* Build a 4-option MCQ around a known-correct numeric value. */
function mcqFromNumber(qText, correct) {
    const options = new Set([correct]);
    let guard = 0;
    while (options.size < 4 && guard < 50) {
        guard++;
        const delta = randInt(1, Math.max(2, Math.round(Math.abs(correct) * 0.25) + 3));
        options.add(correct + (Math.random() < 0.5 ? delta : -delta));
    }
    const opts = shuffle(Array.from(options)).map(String);
    return { q: qText, options: opts, answer: opts.indexOf(String(correct)) };
}

/* Real, randomly-generated arithmetic questions, scaled to grade level and
   matched to the topic name (addition, fractions, place value, etc). Falls
   back to a conceptual prompt for topics with no clean numeric form
   (geometry, patterns, measurement...). */
function generateMathQuestion(grade, subject, topic, difficulty = "medium") {
    const g = grade === "K" ? 0 : parseInt(grade, 10) || 0;
    const scale = Math.max(1, Math.min(g, 10));

    // Apply difficulty multiplier (15x scaling for each difficulty level)
    ...
    const range = (5 + scale * (scale + 1) * 5) * difficultyMultiplier;
    const t = topic.toLowerCase();

    if (t.includes("add")) {
        const a = randInt(1, range), b = randInt(1, range);
        return mcqFromNumber(`What is ${a} + ${b}?`, a + b);
    }
    ...
    if (t.includes("multipli")) {
        const a = randInt(2, Math.max(3, (scale + 2) * difficultyMultiplier)), b = randInt(2, Math.max(3, (scale + 4) * difficultyMultiplier));
        return mcqFromNumber(`What is ${a} × ${b}?`, a * b);
    }
    if (t.includes("divi")) {
        const b = randInt(2, Math.max(3, (scale + 2) * difficultyMultiplier)), q = randInt(2, Math.max(3, (scale + 4) * difficultyMultiplier));
        return mcqFromNumber(`What is ${b * q} ÷ ${b}?`, q);
    }
    if (t.includes("fraction")) {
        const den = randInt(2, Math.max(10, 10 * difficultyMultiplier)), num1 = randInt(1, den - 1), num2 = randInt(1, den - 1);
        const correct = num1 + num2 > den ? "Greater than 1" : (num1 + num2 === den ? "Exactly 1" : "Less than 1");
        const options = shuffle(["Greater than 1", "Exactly 1", "Less than 1"]);
        return { q: `Is ${num1}/${den} + ${num2}/${den} greater than, less than, or exactly 1?`, options, answer: options.indexOf(correct) };
    }
    if (t.includes("percent")) {
        const pct = [5, 10, 20, 25, 50][randInt(0, 4)];
        const base = randInt(2, Math.max(20, 20 * difficultyMultiplier)) * 10;
        return mcqFromNumber(`What is ${pct}% of ${base}?`, Math.round(base * pct / 100));
    }
    ...
    if (t.includes("decimal")) {
        const a = randInt(1, range * 10) / 10, b = randInt(1, range * 10) / 10;
        return mcqFromNumber(`What is ${a} + ${b}? (round to 1 decimal place)`, Math.round((a + b) * 10) / 10);
    }
    if (t.includes("place value") || t.includes("number")) {
        const n = randInt(10, Math.max(100, range));
        const digits = String(n).split("");
        const pos = randInt(0, digits.length - 1);
        const placeNames = ["ones", "tens", "hundreds", "thousands", "ten-thousands"];
        const placeName = placeNames[digits.length - 1 - pos] || "ones";
        return mcqFromNumber(`In the number ${n}, what is the digit in the ${placeName} place?`, parseInt(digits[pos], 10));
    }
    if (t.includes("exponent") || t.includes("cube") || t.includes("square root") || t.includes("radical")) {
        const base = randInt(2, Math.max(3, Math.min(12, scale + 2)));
        const isCube = t.includes("cube");
        const power = isCube ? 3 : 2;
        return mcqFromNumber(`What is ${base}${isCube ? "³" : "²"} (${base} ${isCube ? "cubed" : "squared"})?`, Math.pow(base, power));
    }
    if (t.includes("perimeter") || t.includes("area") || t.includes("mensuration")) {
        const w = randInt(2, Math.max(5, scale * 2)), h = randInt(2, Math.max(5, scale * 2));
        const wantArea = t.includes("area") || (t.includes("mensuration") && Math.random() < 0.5);
        return wantArea
            ? mcqFromNumber(`A rectangle is ${w} cm long and ${h} cm wide. What is its area (in cm²)?`, w * h)
            : mcqFromNumber(`A rectangle is ${w} cm long and ${h} cm wide. What is its perimeter (in cm)?`, 2 * (w + h));
    }
    if (t.includes("shape") || t.includes("geometry") || t.includes("quadrilateral")) {
        const shapes = [["triangle", 3], ["square", 4], ["rectangle", 4], ["pentagon", 5], ["hexagon", 6]];
        const [name, sides] = shapes[randInt(0, shapes.length - 1)];
        return mcqFromNumber(`How many sides does a ${name} have?`, sides);
    }
    if (t.includes("data") || t.includes("statistic") || t.includes("average") || t.includes("mean")) {
        const count = randInt(3, 5);
        const nums = Array.from({ length: count }, () => randInt(1, Math.max(10, range)));
        const sum = nums.reduce((a, b) => a + b, 0);
        return mcqFromNumber(`What is the average (mean) of these numbers: ${nums.join(", ")}?`, Math.round(sum / count));
    }
    if (t.includes("probab")) {
        const total = randInt(4, 10);
        const favorable = randInt(1, total - 1);
        const gcdVal = (a, b) => b === 0 ? a : gcdVal(b, a % b);
        const g = gcdVal(favorable, total);
        const options = shuffle([`${favorable / g}/${total / g}`, `${total}/${favorable}`, `${favorable}/${total + 1}`, `${favorable + 1}/${total}`]);
        const correctText = `${favorable / g}/${total / g}`;
        return { q: `A bag has ${total} balls, of which ${favorable} are red. What is the probability of picking a red ball, in simplest form?`, options, answer: options.indexOf(correctText) };
    }
    return genericConceptQuestion(grade, subject, topic, difficulty);
}

/* Category-flavored, structurally-valid MCQ prompts for any subject/topic
   that has no curated bank and no clean numeric form. Cycled with light
   randomization so a large question count doesn't look identical. */
function genericConceptQuestion(grade, subject, topic, difficulty = "medium") {
    const gLabel = gradeLabel(grade);
    const s = subject.toLowerCase();
    let verb = "study", verb2 = "explain";
    if (s.includes("science") || s.includes("evs")) { verb = "observe and investigate"; verb2 = "explain"; }
    else if (s.includes("social")) { verb = "analyze"; verb2 = "describe"; }
    else if (s.includes("hindi") || s.includes("sanskrit") || s.includes("language") || s.includes("english")) { verb = "read and use"; verb2 = "identify"; }
    else if (s.includes("motor") || s.includes("emotional")) { verb = "practice"; verb2 = "demonstrate"; }
    else if (s.includes("math") || s.includes("numeracy")) { verb = "work with"; verb2 = "apply"; }

    function build(q, correctText, distractors) {
    ...
    const templates = [
        () => build(
            `Which of the following best describes '${topic}' as covered in ${gLabel} ${subject}?`,
            `A concept ${gLabel} students ${verb} as part of ${subject}`,
            [
                `A topic that belongs to a completely different subject`,
                `A concept taught only in much higher grades`,
                `A term with no defined meaning in ${subject}`
            ]
        ),
        () => ({
            q: `True or False: '${topic}' is part of the ${gLabel} ${subject} curriculum.`,
            options: ["True", "False"],
            answer: 0
        }),
        () => build(
            `A student who understands '${topic}' should be able to ${verb2} it in the context of ${subject}. What is the best next study step?`,
            `Review examples of '${topic}' and practice applying them`,
            [
                `Skip '${topic}' since it isn't part of ${subject}`,
                `Memorize an unrelated topic instead`,
                `Wait until next year to learn about it`
            ]
        ),
        () => build(
            `Which subject is '${topic}' most closely associated with?`,
            subject,
            ["Physical Education", "Music", "Art"].filter(v => v !== subject)
        )
    ];

    // For harder difficulties, add more complex analysis questions
    if (difficulty === "hard") {
        templates.push(
            () => build(
                `How can understanding '${topic}' help in applying concepts beyond ${subject}?`,
                `It provides foundational knowledge transferable to other disciplines`,
                [
                    `It only applies to ${subject} and has no broader application`,
                    `It is too advanced for any practical application`,
                    `It contradicts concepts learned in other subjects`
                ]
            ),
            () => build(
                `Which statement best reflects a critical analysis of '${topic}' in ${gLabel} ${subject}?`,
                `Understanding both the theory and practical applications of '${topic}' is essential`,
                [
                    `Memorizing facts about '${topic}' is sufficient for mastery`,
                    `'${topic}' is outdated and no longer relevant`,
                    `'${topic}' requires only surface-level understanding`
                ]
            )
        );
        ...
        templates.push(
            () => build(
                `Which is the most important aspect of '${topic}' to understand?`,
                `How it connects to other concepts in ${subject}`,
                [
                    `Only its name and definition`,
                    `Its historical origin only`,
                    `The exact page number it appears on`
                ]
            )
        );
    }
...
/* Whether the curated ELA bank actually has real content for this
   grade/subject/topic combination. */
function hasCuratedContent(grade, subject, topic) {
    return subject === "English" && !!(ELA_QUIZ_BANK[grade] && ELA_QUIZ_BANK[grade].topics[topic]);
}

/* Entry point for the Create Quiz flow: works for ANY grade/subject/topic
   pulled from CBSE_SYLLABUS, and can produce as many questions as requested
   (up to the 1-100 range enforced by the UI) since math/concept questions
   are generated on demand rather than drawn from a fixed pool. */
function getCreateQuizQuestions(grade, subject, topic, count, difficulty = "medium") {
    if (hasCuratedContent(grade, subject, topic)) {
        return getQuestionsFor(grade, topic, count);
    }
    const wanted = Math.max(1, Math.min(count || 1, 100));
    const isMath = subject === "Mathematics" || subject === "Numeracy";
    const items = [];
    for (let i = 0; i < wanted; i++) {
        items.push(isMath ? generateMathQuestion(grade, subject, topic, difficulty) : genericConceptQuestion(grade, subject, topic, difficulty));
    }
    return items;
}
...
function removeTextbook() {
    ...
    document.getElementById("textbookUpload").value = "";
    document.getElementById("textbookLinkInput").value = "";
    document.getElementById("textbookPreview").style.display = "none";
}

function handleLinkAnalysis() {
    const urlInput = document.getElementById("textbookLinkInput");
    ...
    statusEl.textContent = "Fetching and analyzing content from: " + url;

    // Fetch the URL with CORS-friendly approach
    fetch(url, {
        method: "GET",
        headers: {
        ...
        if (!response.ok) {
            throw new Error("HTTP " + response.status + ": Could not fetch the page. Make sure the URL is public and accessible.");
        }
        ...
        if (!text || text.length < 50) {
            alert("The page doesn't contain enough readable text, or it might be JavaScript-heavy. Try copying the text directly and uploading as a .txt file instead.");
            statusEl.textContent = "Upload .txt/.pdf or paste a URL. Attached as reference material for this quiz — shown to students on the quiz link and printed on the PDF so they know what to study from.";
            return;
        }

        // Apply the extracted text as textbook
        applyTextbookText("Web Link: " + url, text);
    }).catch(err => {
        console.error("Link analysis error:", err);
        alert("Could not fetch the URL. This might be due to:\n1. URL is invalid or unreachable\n2. Site blocks external requests (CORS)\n3. Site requires authentication\n\nTry copying the text manually and uploading as a .txt file instead.");
        statusEl.textContent = "Upload .txt/.pdf or paste a URL. Attached as reference material for this quiz — shown to students on the quiz link and printed on the PDF so they know what to study from.";
    });
}
...
function extractTextFromHTML(html) {
    ...
    // Return up to MAX_TEXTBOOK_CHARS characters
    return cleaned.slice(0, MAX_TEXTBOOK_CHARS);
}

function populateQuizSubjects() {
    const grade = document.getElementById("quizGrade").value;
    fillSubjectSelect(document.getElementById("quizSubject"), grade);
    // Reset topic when grade changes
    document.getElementById("quizTopic").innerHTML = `<option value="">-- Select a subject first --</option>`;
}
...
function populateQuizTopics() {
    ...
    // English draws its topic list straight from the curated ELA bank (not
    // the CBSE syllabus list) so the selected topic name always matches a
    // real set of curated questions — otherwise every English quiz would
    // silently fall back to generic, unrelated filler questions.
    if (subject === "English" && ELA_QUIZ_BANK[grade]) {
        fillTopicSelect(topicEl, grade);
    } else {
        fillCBSETopicSelect(topicEl, grade, subject);
    }
}
...
function generateQuestions() {
    ...
    const questions = getCreateQuizQuestions(grade, subject, topic, numQ, currentQuizDifficulty);
    ...
    // Show textbook summary if attached
    const textbookSummaryEl = document.getElementById("quizTextbookSummary");
    const textbookInfoEl = document.getElementById("quizTextbookInfo");
    if (currentTextbook) {
        textbookSummaryEl.style.display = "block";
        const notes = (document.getElementById("quizNotes").value || "").trim();
        textbookInfoEl.innerHTML =
            `<strong>File:</strong> ${escapeHtml(currentTextbook.fileName)} (${currentTextbook.text.length.toLocaleString()} characters)` +
            (notes ? `<br><strong>Section focus:</strong> ${escapeHtml(notes)}` : "");
    } else {
        textbookSummaryEl.style.display = "none";
    }
...
async function init() {
    ...
    // Initialize CBSE subject and topic dropdowns
    const quizSubjectEl = document.getElementById("quizSubject");
    const quizTopicEl = document.getElementById("quizTopic");
    if (quizSubjectEl) quizSubjectEl.innerHTML = `<option value="">-- Select Subject --</option>`;
    if (quizTopicEl) quizTopicEl.innerHTML = `<option value="">-- Select a subject first --</option>`;
