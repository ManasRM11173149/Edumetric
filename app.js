...
function generateMathQuestion(grade, subject, topic) {
    const g = grade === "K" ? 0 : parseInt(grade, 10) || 0;
    const scale = Math.max(1, Math.min(g, 10));
    const range = 5 + scale * (scale + 1) * 5;
    ...
    if (t.includes("multipli")) {
        const a = randInt(2, Math.max(3, scale + 2)), b = randInt(2, Math.max(3, scale + 4));
        return mcqFromNumber(`What is ${a} × ${b}?`, a * b);
    }
    if (t.includes("divi")) {
        const b = randInt(2, Math.max(3, scale + 2)), q = randInt(2, Math.max(3, scale + 4));
        return mcqFromNumber(`What is ${b * q} ÷ ${b}?`, q);
    }
    if (t.includes("fraction")) {
        const den = randInt(2, 10), num1 = randInt(1, den - 1), num2 = randInt(1, den - 1);
    ...
    return genericConceptQuestion(grade, subject, topic);
}
...
function genericConceptQuestion(grade, subject, topic) {
...
function getCreateQuizQuestions(grade, subject, topic, count) {
    ...
    for (let i = 0; i < wanted; i++) {
        items.push(isMath ? generateMathQuestion(grade, subject, topic) : genericConceptQuestion(grade, subject, topic));
    }
    return items;
}
...
function generateQuestions() {
    ...
    const questions = getCreateQuizQuestions(grade, subject, topic, numQ);
