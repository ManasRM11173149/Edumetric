...
let state = {
    ...
    savedQuizzes: [],  // [ {id,title,grade,subject,topic,questions:[...]} ]
...
function generateQuestions() {
    ...
    const subject = document.getElementById("quizSubject") ? document.getElementById("quizSubject").value : ""; // NEW: Subject from CBSE curriculum
    ...
    document.getElementById("questionsSection").dataset.subject = subject; // NEW: Store subject
...
function collectQuizFromEditor(title, grade, subject, topic) {
    const questions = [];
...
function saveQuiz() {
    ...
    const subject = document.getElementById("questionsSection").dataset.subject || ""; // NEW: Get subject
    const topic = document.getElementById("questionsSection").dataset.topic || "";
    const quiz = collectQuizFromEditor(title, grade, subject, topic);
...
function hostQuiz(id) {
    ...
    const payload = {
        title: quiz.title,
        grade: quiz.grade,
        subject: quiz.subject || "", // NEW: Include subject
        topic: quiz.topic,
...
function printSpecificQuiz(id) {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
        ...
        <div class="meta">${quiz.subject ? escapeHtml(quiz.subject) : "English / Language Arts"}${quiz.grade ? " — " + gradeLabel(quiz.grade) : ""}${quiz.topic && quiz.topic !== "__ALL__" ? " — " + escapeHtml(quiz.topic) : ""}</div>
...
function createStudentQuiz() {
    ...
    const subject = document.getElementById("studentQuizSubject") ? document.getElementById("studentQuizSubject").value : ""; // NEW: Subject from CBSE
    ...
    state.savedQuizzes.push({ id: "quiz-" + Date.now(), title, grade, subject, topic, questions }); // NEW: Added subject
    quizzesCreated = state.savedQuizzes.length;
    saveState();
    updateDashboard();
    updateSavedQuizzesList();
...
function buildClassroomContext() {
    ...
    const quizTitles = state.savedQuizzes.map(q => `"${q.title}" (${gradeLabel(q.grade)}${q.subject ? ", " + q.subject : ""}, ${q.topic})`).join("; ");
